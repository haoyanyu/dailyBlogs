...
const groupData = (data, groupSizeParams) => {
  const groupDataAction = (originData, groupSize) => {
    ...
    originData.forEach((id, index) => {
      if ((index + 1) % groupSize === 0) {
        group.push(id)
        console.log(`%c >>>>>>第${count}组 ${group.length}条<<<<<<`, 'font-weight: bold', group.join(','));
        count++;
        group = [];
      } else {
        group.push(id);
        if (index + 1 === originData.length) {
          console.log(`%c >>>>>>第${count}组 最后${group.length}条<<<<<<`, 'font-weight: bold', group.join(','));
        }
      }
    })
  }
  if (!Array.isArray(groupSizeParams)) {
    ...
    groupSizeParams.forEach((groupSize, index) => {
      console.log(`%c >>>>>>第${index+1}次分组，每组${groupSize}条数据<<<<<<`, 'color: #07828B; font-weight: bold; font-size: 14px');
...
export default function ReadCsv() {
  ...
  // 处理表单数据
  const handleSizeChange = useCallback((key) => (value) => {
    if (key === 'groupSizeOne') {
  ...
  const handleSubmit = useCallback(() => {
    ...
    files.forEach(file => {
      ...
      reader.onload = () => {
        ...
        console.log(`%c >>>>>>${file.name}的数据为<<<<<<`, 'color: #CB272D; font-size: 16px', datas.join(','));
        groupData(datas, needGroupTwice ? [groupSizeOne, groupSizeTwo] : groupSizeOne);
      }
    })
  ...
  return (
    <Form onSubmit={handleSubmit}>
      <Form.Item label="是否需要分组两次">
        <Radio.Group onChange={handleSizeChange('needGroupTwice')} defaultValue={1}>
          <Radio value={1}>是</Radio>
          <Radio value={0}>否</Radio>
        </Radio.Group>
      </Form.Item>
      <Form.Item label="一级分组大小">
        <Radio.Group onChange={handleSizeChange('groupSizeOne')} defaultValue='50'>
...

      {
        !!needGroupTwice && (
          <Form.Item label="二级分组大小">
            <Radio.Group onChange={handleSizeChange('groupSizeTwo')} defaultValue='50'>
              <Radio value="50">50</Radio>
              <Radio value="100">100</Radio>
            </Radio.Group>
          </Form.Item>
      ...
      <Form.Item label="选择文件">
        <div className={style.customUploadWrapper}>
          <input type="file" accept='.csv' multiple onChange={handleFileChange} className={style.customUploadInput} />
          <Button
            type='primary'
          ...
          {!!files.length && (
            <div className={style.customUploadList}>
              <List
                size="small"
                dataSource={files}
                render={(item) => <List.Item key={item.name}>{item.name}</List.Item>}
              />
            </div>
          )}
        </div>
